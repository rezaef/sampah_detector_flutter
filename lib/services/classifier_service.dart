...
class ClassifierService {
  ...
  _ImageFeatures _extractFeatures(img.Image image, img.Image edges) {
    ...
    for (var y = 0; y < image.height; y++) {
      for (var x = 0; x < image.width; x++) {
        ...
        final isBrown = hsv.h >= 15 && hsv.h <= 55 && hsv.s >= 0.20 && hsv.v <= 0.85;
    ...
    final variance = max(0.0, (luminanceSquaredSum / totalPixels) - (meanLuminance * meanLuminance));
  ...
  double _clampProbability(double value, {double minValue = 0.0, double maxValue = 1.0}) {
    if (value < minValue) {
      return minValue;
    }
    if (value > maxValue) {
